...
//     test(new BubbleFifo(UInt(32.W), 4)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
//       testFn(dut)
//     }
//   }

  "FifoRegister" should "pass" in {
    test(new FifoRegister(UInt(32.W))).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      testFn(dut)
    }
  }
